...
{% for page in site.pages %}
	{% unless page.path contains 'zh-CN/' %}
	{% if page.include_in_search_results and page.title %}
		...
		{% for it_page in site.pages %}
			{% unless it_page.path contains 'zh-CN/' %}
...
function displaySearchResults(has_search_text, results, pages) {
	...
	if (results.length && has_search_text) {
		...
		for (var i = 0; i < results.length && i < max_results + num_elements_and_properties; i++) {
			...
			function highlightMatches(content, positions, skip_after_index) {
				...
				for (var j = 0; j < positions.length; j++) {
					...
					new_content += content.slice(cursor, pos[0])
						+ '<strong>'
			...
			if (content_positions.length) {
				...
				var summary_begin = Math.max(
					0,
					content.lastIndexOf(' ', Math.max(0, first_match - 60))
				);
				...
				summary_end = Math.max(
					...
					i_strong < 0 ? -1 : i_strong + '</strong>'.length
				);
				...
				content = content.substring(
					0,
					Math.max(summary_length, content.indexOf(' ', summary_length))
				);
			}
			...
			if (type == "property") {
				results_string += '<h4 title="RCSS property"><span class="fas">&#xf121;</span>'
					+ url + '‘' + title + '’ property</a></h4>';
			} else if (type == "element") {
				results_string += '<h4 title="RML element"><span class="fas">&#xf0ce;</span>'
					+ url + '&lt;' + title + '&gt; element</a></h4>';
			} else if (type == "pseudo") {
				results_string += '<h4 title="Pseudo selector"><span class="far">&#xf192;</span>'
					+ url + ' ‘:' + title + '’ pseudo selector</a></h4>';
			} else {
				results_string += '<h4>'
					+ url
					+ title
		...
		results_string += '<p style="text-align: right"><em>Showing '
			+ Math.min(results.length, max_results + num_elements_and_properties)
			+ ' of '
			+ results.length
			+ ' '
